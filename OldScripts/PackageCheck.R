...
for (i in 1:num_folds) {
  ...
  bike_example <- fnn.fit(resp = train_y, 
  ...
  # Predicting
  pred_fnn = fnn.predict(bike_example,
                         bike_data_test, 
                         scalar_cov = NULL,
                         basis_choice = c("bspline"), 
                         num_basis = c(3),
...
Final_Table_Bike2 = matrix(nrow = 8, ncol = 2)

# Collecting errors
Final_Table_Bike2[1, ] = colMeans(error_mat_lm, na.rm = T)
Final_Table_Bike2[2, ] = colMeans(error_mat_np, na.rm = T)
Final_Table_Bike2[3, ] = colMeans(error_mat_pc1, na.rm = T)
Final_Table_Bike2[4, ] = colMeans(error_mat_pc2, na.rm = T)
Final_Table_Bike2[5, ] = colMeans(error_mat_pc3, na.rm = T)
Final_Table_Bike2[6, ] = colMeans(error_mat_pls1, na.rm = T)
Final_Table_Bike2[7, ] = colMeans(error_mat_pls2, na.rm = T)
Final_Table_Bike2[8, ] = colMeans(error_mat_fnn, na.rm = T)
